...
let rec lookup: 'a -> ('a * 'b) list -> ('a * 'b) option = fun k dict ->
        match dict with
        | [] -> None
...
let comb_lists: 'a list -> 'b list -> ('a * 'b) list option =
        let rec helper = fun accum a b ->
                match (a, b) with
                | [], [] -> Some accum
                | _, [] | [], _ -> None (* unequal lengths *)
                | x::xs, y::ys -> helper (accum @ [(x,y)]) xs ys
        in fun a b -> helper [] a b;;

let split_dict: ('a * 'b) list -> ('a list * 'b list) =
        let rec helper = fun ak av rem ->
                match rem with
                | [] -> (ak, av)
                | (k,v)::xs ->helper (ak @ [k]) (av @ [v]) xs
        in fun lst -> helper [] [] lst;;

let try_add: ('a * 'b) list -> 'a -> 'b -> ('a * 'b) list = fun dict k v ->
        match add k v dict with Some v -> v | None -> dict;;

let rec union: ('a * 'b) list -> ('a * 'b) list -> ('a * 'b) list = fun l r ->
        match r with
        | [] -> l
        | (k,v)::xs -> union (try_add l k v) xs;;

let mk_dict: ('a * 'b) list -> ('a * 'b) list =
        fun l -> union [] l;;
