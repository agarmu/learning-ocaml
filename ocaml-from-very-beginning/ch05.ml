...
let sorted' : 'a list -> bool =
  let rec srt' : 'a -> 'a list -> bool =
   fun v l -> match l with [] -> true | x :: xs -> v <= x && srt' x xs
  in
  fun lst -> match lst with [] -> true | x :: xs -> srt' x xs
...
let insert : 'a -> 'a list -> 'a list =
  let rec ins_helper v left_acc right_rem =
    ...
    | [] -> left_acc
    | x :: xs ->
        if x < v then ins_helper v (left_acc @ [ x ]) xs
        else left_acc @ [ v ] @ right_rem
  in
  fun v l -> ins_helper v [] l

let rec isort l = match l with [] -> [] | x :: xs -> insert x (isort xs)

(* merge sort *)
let merge : 'a list -> 'a list -> 'a list =
  let rec helper accum x y =
    match (x, y) with
    | [], _ -> accum @ y
    | _, [] -> accum @ x
