let smallest : 'a list -> 'a =
  let rec h : 'a -> 'a list -> 'a =
   fun cur l ->
    match l with [] -> cur | x :: xs -> h (if x < cur then x else cur) xs
  in
  fun l -> match l with [] -> raise Not_found | x :: xs -> h x xs
  ...
  let rec guess_sm_sqrt : int -> int -> int -> int =
   fun v l u ->
    ...
    if l = u then l
      ...
      if guess <= v then guess_sm_sqrt v mid u else guess_sm_sqrt v l (mid - 1)
  in
