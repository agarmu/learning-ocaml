...
(* tail-recursively *)
let rec rev_helper : 'a list -> 'a list -> 'a list =
 fun acc lst ->
  match lst with [] -> acc | x :: xs -> rev_helper ([ x ] @ acc) xs
...
let rec take n l =
  if n <= 0 then []
  else match l with x :: xs -> x :: take (n - 1) xs | _ -> []

let rec drop n l =
  if n <= 0 then l else match l with x :: xs -> drop (n - 1) xs | _ -> []

(*
exercises
*)
let rec even : 'a list -> 'a list =
 fun l -> match l with _ :: x :: xs -> x :: even xs | _ -> []

let rec cttrue : bool list -> int =
 fun l -> match l with [] -> 0 | x :: xs -> if x then 1 else 0 + cttrue xs

let rec cttrue_tailrec n l =
  match l with [] -> n | x :: xs -> cttrue_tailrec (n + if x then 1 else 0) xs
...
let rec mkpalindrome : 'a list -> 'a list =
 fun l -> match l with [] -> [] | x :: xs -> [ x ] @ mkpalindrome xs @ [ x ]

let rec mkpalindrome' : 'a list -> 'a list = fun l -> l @ rev' l
let rec ispalindrome (l : 'a list) : bool = l = rev l

let rec droplast l =
  match l with [] | [ _ ] -> [] | x :: xs -> x :: droplast xs
...
let rec member v l =
  match l with [] -> false | x :: xs -> x = v || member v xs

let rec mkset_help : 'a list -> 'a list -> 'a list =
 fun acc l ->
  match l with
  | [] -> acc
  | x :: xs ->
      let acc' = if member x acc then acc else x :: acc in
      mkset_help acc' xs
