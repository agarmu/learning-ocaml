...
let rec msort: ('a -> 'a -> bool) -> 'a list -> 'a list = fun cmp l ->
        match l with
        | [] | [_] -> l
        | _ -> let x, y = split l in merge cmp (msort cmp x) (msort cmp y);;
...
let calm: char list -> char list =
        let f: char -> char = fun x -> if x = '!' then '.' else x
        in map f;;

let clip: int list -> int list =
        let f: int -> int -> int -> int = fun lower upper value ->
                if value < lower then lower else if value > upper then upper
                else value
        in map (f 0 100);;

let rec apply: ('a -> 'a) -> int -> 'a -> 'a = fun f n v ->
        if n = 1 then
                f v
        else f (apply f (n-1) v);;

let insert: ('a -> 'a -> bool) -> 'a -> 'a list -> 'a list =
        let rec ins_helper cmp v left_acc right_rem =
                match right_rem with
                  [] -> left_acc
                | x::xs -> if cmp x v then
                        ins_helper cmp v (left_acc @ [x]) xs
                        else left_acc @ [v] @ right_rem
        in fun cmp v l -> ins_helper cmp v [] l;;

let rec isort: ('a -> 'a -> bool) -> 'a list -> 'a list = fun cmp l ->
        match l with
        [] -> []
        | x::xs -> insert cmp x (isort cmp xs);;

let rec splitq: ('a -> 'a -> bool) -> 'a -> 'a list -> 'a list -> 'a list -> 'a
list * 'a list = 
...
let rec qsort: ('a -> 'a -> bool) -> 'a list -> 'a list = fun cmp l ->
        match l with
        [] -> []
        | x::xs -> let l, r = splitq cmp x [] [] xs in
...
let filter: ('a -> bool) -> 'a list -> 'a list = 
        ...
        in fun f l -> helper f [] l;;

let rec for_all: ('a -> bool) -> 'a list -> bool = fun f l ->
        match l with [] -> true | x::xs -> (f x) && for_all f xs;;

let mapl: ('a -> 'b) -> 'a list list -> 'b list list = fun f ->
        map (map f);;
